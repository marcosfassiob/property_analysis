...
### Does Age make a difference on in the Sale Value?
...
Since years were on an interval of time, I needed to reduce and categorize them. I deccided to make a new column called "Time.Period" and had three categories: "Old" would be any year before 1995. "Modern" would any year from 1995 to 2019, and anything greater than 2019 would "New". To help visualize the data, I reduced and sampled 5000 data points. I proceeded to follow the kNN steps: doing a 70/30 split, running it on the training data. I chose a k = 5, constructed the confusion matrix to get an accuracy of 76%. 

A problem doing this method is that the sales are not distributed evenly through the years. Before 1980, the property sales were either not as frequent or were not logged. In modern times, the property sales have are accounted for a lot more. So there is a heavy bias towards classifying something as Modern since a good portion of the sales are from that time period.
...
ggplot(temp, aes(Predicted, Reference, fill=Frequency)) + theme_bw() +
    ...
    labs(title="Predicted vs. Actual for Time Period")
```
